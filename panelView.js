...
const PANEL_COLOR = 0x5865f2; // Discord blurple — regular temp-vc panel
...
function buildPanelEmbed(ownerMember, tempData) {
  ...
  return new EmbedBuilder()
    ...
    .setTitle(`${tempData.emoji || '🔊'} Channel Controls`)
    .setDescription(`Owned by **${ownerName}**`)
    .addFields(
      { name: 'Status', value: lockedText, inline: true },
      { name: 'User Limit', value: limitText, inline: true },
      { name: 'Trusted Users', value: `${trustedCount}`, inline: true },
      { name: 'Chat Auto-Clean', value: cleanupText, inline: true },
    )
    .setFooter({ text: 'Use the buttons below to manage this channel' });
}
