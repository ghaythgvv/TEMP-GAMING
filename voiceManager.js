...
async function createTempChannel(member, guild, config) {
  ...
  // apply the blocked-role deny (if configured) on top, then finally the
  // owner's own overwrite — a member-specific overwrite always beats a
  // role-specific one, so this order guarantees the owner is never the one
  // who ends up locked out by either of the previous two steps.
...
async function createGameChannel(member, guild, config) {
  ...
  // Same ordering as createTempChannel above: category sync, then the
  // blocked-role deny, then the owner's own overwrite last so it always
  // wins.
  await syncToCategoryPermissions(channel);
  await denyBlockedRole(channel, config);
  await updateOwnerPermissions(channel, null, member.id);
