...
// Optional: mention a game's associated Discord role (e.g. a "Valorant"
// role) in the channel whenever that game gets picked, so anyone with that
// role gets pinged that someone's looking to play. Configure this by
// setting config.gameRoleIds = { valorant: 'ROLE_ID', amongus: 'ROLE_ID', ... }
// (same place as categoryId/gameCategoryId) — send me the role IDs for
// whichever games have one and I'll fill this in. Until then, this quietly
// does nothing for every game.
function announceGameRole(channel, guildId, gameKey) {
  if (!gameKey) return;
...
async function handleGameInteraction(interaction) {
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId.startsWith('game_pick_')) {
      ...
      storage.setTempChannel(channel.id, tempData);
      announceGameRole(channel, interaction.guild.id, game.key);
