...
function buildGamePanelEmbed(member, tempData) {
  ...
  const statusLine = `${lockedText} • 👥 **Limit:** ${limitText}`;
  const ownerLine = `👑 **Owner:** ${ownerName}`;
  ...
  embed
    ...
    .setDescription(
      [ownerLine, statusLine, '### Now Playing', startedLine, extraLine, '*Owner-only controls above.*']
        .filter(Boolean)
        .join('\n\n')
    );
...
function buildRoomControlsRow(tempData) {
...
function buildGamePanelComponents(tempData) {
  ...
  if (tempData.game) {
    ...
    return [new ActionRowBuilder().addComponents(buttons), buildRoomControlsRow(tempData)];
  }
  ...
  // own row, however many games end up on the list. This maxes out at 4
  // rows with the current 12-game list + "Others", leaving exactly one
  // more row (Discord's 5-row cap) for Lock/Limit below it.
  const allButtons = [...GAME_LIST, { key: 'other', label: 'Others', emoji: '➕' }];
  const gameRows = chunkEvenly(allButtons, BUTTONS_PER_ROW).map((chunk) =>
  ...
  return [...gameRows, buildRoomControlsRow(tempData)];
}
